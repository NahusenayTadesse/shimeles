<script lang="ts">
	...
	import { BookOpen, Compass, Quote, Target } from '@lucide/svelte';
	 ...
	 * only the paragraphs, the two hero photographs and the memorial gallery
	 * come from the database (`about_content` / `about_gallery_images`, edited
...
<div class="mx-auto flex w-full max-w-6xl flex-col gap-20 px-4 py-16 md:gap-28 md:py-24">
	...
	<section use:reveal class="mx-auto flex max-w-3xl flex-col gap-6">
		...
		{:else}
			<p class="text-muted-foreground">
				This foundation carries the name of Shimeles Abera, and it was built by the people who
				knew him.
			</p>
		{/if}
	</section>
	...
	<section class="flex flex-col gap-8">
		...
		<div class="grid gap-6 md:grid-cols-2">
			<div use:reveal={{ delay: stagger(0, 100, 2) }} class="flex flex-col gap-4 rounded-[2rem] border bg-card p-8 shadow-warm">
				<div
			...
			<div use:reveal={{ delay: stagger(1, 100, 2) }} class="flex flex-col gap-4 rounded-[2rem] border bg-card p-8 shadow-warm">
				<div
					class="group flex size-16 items-center justify-center rounded-full bg-clay-deep text-olive ring-4 ring-olive/15 transition-all duration-300 ease-out hover:scale-110 hover:rotate-6 hover:ring-olive/30"
				>
					<Compass class="size-7 transition-transform duration-300 ease-out group-hover:scale-110" />
				</div>
	...
	<section class="mx-auto flex max-w-2xl justify-center">
		<div class="tilt-left relative rounded-[2rem] bg-card px-8 py-12 text-center shadow-warm sm:px-14">
			<Quote class="absolute top-4 left-6 size-16 text-terracotta/15 sm:size-20" fill="currentColor" />
...
<section id="in-memoriam" class="scroll-mt-20 py-16 md:py-24">
	...
	<div class="relative w-full overflow-hidden bg-clay-deep shadow-warm">
		<div
				...
				{#if memoriamMainPhoto}
					{#key memoriamMainPhoto.storagePath}
						<img
							...
							class="h-[24rem] w-full rounded-[1.75rem] object-cover shadow-warm sm:h-[30rem] lg:h-[36rem]"
						/>
					{/key}
				{/if}
